...
prev=None
for i in range(180):  # up to 360 min
...
for l in full.splitlines():
    if any(k in l for k in ["GPU0","GPU1","RTX","T4","P100","sm_","train.csv","loaded","trainable","step ","Training done","adapter r","CV ","submission.zip","Error","Found no","Total VRAM","Loading"]):
        print(">",l.encode("ascii","replace").decode()[:175],flush=True)
