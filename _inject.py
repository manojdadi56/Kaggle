"""Inject GPU-adaptive training pipeline into the new fork, preserving its metadata.kaggle bindings."""
import json, urllib.request
from pathlib import Path

nb = json.loads(Path("_demo_fork.json").read_text(encoding="utf-8"))
...
"assert cc >= 7.0, (\n"
"    f'GPU {props.name} sm_{props.major}{props.minor} INCOMPATIBLE. Use RTX Pro 6000 or T4 (not P100).')\n"
"import csv\n"
...
# Cell 2: model + LoRA + train (vram-adaptive: bf16 if big GPU, 4-bit if small)
cells.append(code(
"import kagglehub\n"
...
"    except ImportError: subprocess.run([sys.executable,'-m','pip','install','-q','bitsandbytes'], check=False); import bitsandbytes\n"
...
# Cell 4: package submission.zip (adapter files only)
cells.append(code(
"import glob as _g\n"
...
nb["cells"] = cells
Path("_inject_out.json").write_text(json.dumps(nb), encoding="utf-8")
...
# NOTE: NOT triggering a run — there's already a v2 RUNNING in browser. Just QuickSave the code so my code is the next version.
# Actually if there's already a run, don't disturb it. Check status first.
status = call("get_notebook_session_status", {"request":{"userName":"sai1881","kernelSlug":"nvidia-nemotron-submission-demo"}})
print("current status:", status)
print("=== NOT pushing code — letting the currently-RUNNING browser commit finish first ===")
print("if it succeeds, I record the result. if it fails, I push my adaptive code as the next version.")
