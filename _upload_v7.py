...
CORPUS = Path("competitions/nvidia-nemotron-model-reasoning-challenge/data/corpus/v4/corpus.jsonl")
USER = env["KAGGLE_USERNAME"]
SLUG = "nemotron-cot-corpus-v7"

with tempfile.TemporaryDirectory() as td:
    td = Path(td)
    shutil.copy(CORPUS, td / "corpus.jsonl")
    (td / "dataset-metadata.json").write_text(json.dumps({
        "title": "Nemotron CoT corpus v7 (Jules 9-cat)",
        "id": f"{USER}/{SLUG}",
        "licenses": [{"name": "CC0-1.0"}],
        "subtitle": "9-category Jules corpus 3593 verified rows",
        "description": "Jules-generated corpus: all 9 host categories x 400 rows each, 99.8% verified.",
        "keywords": ["reasoning"],
    ...
    try:
        ...
        print("trying version:", str(e)[:200])
        try:
            r = api.dataset_create_version(folder=str(td), version_notes="v7", quiet=True)
            print("version:", str(r)[:300])
        except Exception as e2:
            print("version err:", e2)

# Now push notebook attaching v7
...
if not any("nemotron-cot-corpus-v7" in str(d) for d in ds):
    ds.append({"sourceType":"datasetVersion","sourceId":f"{USER}/{SLUG}"})
    nb["metadata"]["kaggle"]["dataSources"] = ds
print("dataSources count after v7 add:", len(ds))
