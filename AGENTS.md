...
### ⭐ DATA TRUTH (verified 2026-05-31 — do NOT re-derive)
- **train.csv = 9500 rows, columns `id,prompt,answer` ONLY (NO category label).** Classify by the "In Alice's Wonderland, a secret ___" signature phrase → `data/classify/classify.py` (100% agreement on verified rows, 0 unknown).
- **Exactly 6 real categories** (partition all 9500): gravity 1597, bit_manipulation 1602, numeral 1576, cipher 1576, unit_conversion 1594, equation_numeric 1555.
- **`cryptarithm`, `cryptarithm_guess`, `select2reason` DO NOT EXIST in train.csv** (0 rows). Any corpus rows in those categories are FABRICATED synthetic — drop them. (The winner repo's 9-category analysis was a different/older dataset; do not trust it for category coverage here.)
- **Real formats differ from old synthetic**: equation_numeric is SYMBOLIC (`` `!*[{ = '"[` ``) with the separator char as a *hidden per-puzzle operator* — research-grade, DEFERRED; bit_manipulation is 8-bit BINARY (`01010001 -> 11011101`), not hex/xor.
- **Build VERIFIED corpus** = run a deterministic solver on the REAL classified rows, keep ONLY rows whose `\boxed{}` exactly matches gold. Builders in `data/build_v13/`:
  - cipher: harvest the plaintext vocab from ALL prompts' `->` example RHS (77 words, covers 100% of targets), complete unseen-letter words by length+pattern+known-letter constraints → **1568/1576 (99.5%)**.
  - bit_manipulation: search a sigma-family (shift/rot + XOR/AND/OR/ANDNOT/ORNOT pairs, XOR/maj/choice triples) for a rule matching all 8 examples → **975/1602 (60.9%)**. Underdetermined coincidental fits are excluded by the gold-verify filter.
- **v13 corpus** = `data/build_v13/assemble_v13.py` → 4112 rows, 100% verified (cipher 1568 + bit 975 + base gravity/numeral/unit_conv 1569; equation held out). Uploaded as private dataset **`sai1881/nemotron-cot-corpus-v13`** via `kaggle.KaggleApi().dataset_create_new` (MCP resumable upload + create_version are unreliable — use create_new). Attach it (swap out v12) for the improved run.
- **Host scoring (official Evaluation page)**: vLLM, `temperature=0.0` (greedy), `max_tokens=7680`, `top_p=1.0`, `max_model_len=8192`, `max_lora_rank=32`; extract `\boxed{}`, correct if exact-string OR rel-tol 1e-2. Local CV MUST use temp=0.0 to be faithful. (Notebook eval caps at 2048 tokens for speed — answers are short.)
