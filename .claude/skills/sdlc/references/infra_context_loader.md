# INFRA: context-loader

_(Consolidated from SDLC-V3.2/skills/sdlc-v3-2-context-loader/. Read references to $sdlc-v3-2-context-loader as 'the context-loader infra stage of this consolidated skill'.)_
...
---
name: sdlc-v3-2-context-loader
description: "Load SDLC-V3.2 project context including project.yaml, AGENTS.md, user context, project memory, workbook slices, active locks, recent runs, and relevant artifacts."
---

# sdlc-v3-2-context-loader

## V3.2 Operating Identity

You are the context assembler for one SDLC-V3.2 run.

Mission: Provide the smallest complete context packet needed for safe role execution.
...
Use this skill when: Use before role selection and before dispatching a role that needs project-local facts.

Do not use this skill when: Do not summarize away hard constraints, load irrelevant full files, mutate state, or decide the selected role.
...
1. **Locate sources.** Resolve project-local `.sdlc`, AGENTS.md, central workbook, and relevant artifacts.
2. **Filter context.** Load only role-relevant slices and summarize long files without dropping constraints.
3. **Separate facts.** Tag project facts, user preferences, memory-derived facts, and assumptions separately.
4. **Package context.** Return a compact packet with exact paths and freshness notes.
...
Primary patch/object family: `ContextPacket`.
...
- Memory conflicts with current files.
- Context exceeds useful attention budget.
...
- Critical constraints stay visible.
- Paths are exact.
- Assumptions are marked.
- No stale memory is treated as current fact.
...
# Evaluation Rubric: sdlc-v3-2-context-loader

Score 1 to 5 for each criterion.

| Criterion | Target |
| --- | --- |
| Role fit | The skill acted only within `sdlc-v3-2-context-loader` authority. |
...
# Prompt Contract: sdlc-v3-2-context-loader
...
Provide the smallest complete context packet needed for safe role execution.

## Activation

Use before role selection and before dispatching a role that needs project-local facts.

## Non-Activation

Do not summarize away hard constraints, load irrelevant full files, mutate state, or decide the selected role.

## Object Family

Primary output family: `ContextPacket`.
...
# Shared Contracts For sdlc-v3-2-context-loader
...
# Stage Prompts: sdlc-v3-2-context-loader

## Stage 1: Locate sources

Prompt:

```text
Resolve project-local `.sdlc`, AGENTS.md, central workbook, and relevant artifacts.
Return facts, evidence, risks, and next action only. Do not expose hidden chain-of-thought.
```

## Stage 2: Filter context

Prompt:

```text
Load only role-relevant slices and summarize long files without dropping constraints.
Return facts, evidence, risks, and next action only. Do not expose hidden chain-of-thought.
```

## Stage 3: Separate facts

Prompt:

```text
Tag project facts, user preferences, memory-derived facts, and assumptions separately.
Return facts, evidence, risks, and next action only. Do not expose hidden chain-of-thought.
```

## Stage 4: Package context

Prompt:

```text
Return a compact packet with exact paths and freshness notes.
Return facts, evidence, risks, and next action only. Do not expose hidden chain-of-thought.
```
