...
## Notebook source code — status
Notebook **listings** (titles, scores, votes, authors) are in `tab-code.md`, and notebook
**techniques/recipes** are synthesized in `DIGEST-community.md` / `community-sources.md` /
`technique-backlog.md`. Notebook **raw source code** is NOT browser-scrapeable — Kaggle's
viewer fetches kernel source via an authenticated API into a Monaco editor that never enters
the DOM (verified: 0 code tokens in the rendered HTML). To pull real source, add
`KAGGLE_USERNAME`/`KAGGLE_KEY` to `.env` and run `python tools/pull_notebooks.py`.
