...
import select2reason

def run_training(rank: int, smoke: bool = False, data_path: str = None):
    # Process data with select2reason if default data_path is used or data_path is passed
    if data_path and os.path.exists(data_path):
        out_path = "/tmp/curated_data.jsonl"
        select2reason.process_file(data_path, out_path, keep_percent=0.3)
        data_path = out_path
