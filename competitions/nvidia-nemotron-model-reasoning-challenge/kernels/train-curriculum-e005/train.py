...
def load_train_data(data_path: str = None) -> list:
    train_data = []
            ...
            with open(data_path, 'r', encoding='utf-8') as f:
                ...
                for row in reader:
                    train_data.append({
                        ...
                        "solver_confidence": float(row.get('solver_confidence', 1.0))
                    })
            ...
            with open(data_path, 'r') as f:
                for line in f:
                    try:
                        d = json.loads(line)
                        train_data.append({
                            "prompt": d.get('prompt', d.get('question', '')),
                            "completion": d.get('completion', d.get('answer', '')),
                            "category": d.get('category', 'unknown'),
                            "solver_confidence": float(d.get('solver_confidence', 1.0))
                        })
                    except:
                        pass
    else:
        # Dummy data for smoke
        train_data = [{"prompt": "What is 2+2?", "completion": "The answer is \\boxed{4}.", "category": "numeral", "solver_confidence": 1.0}] * 10
        print("Using dummy data.")

    # Apply curriculum sort
    train_data = apply_curriculum(train_data, {"enabled": True, "method": "sort_easy_to_hard"})
    return train_data
