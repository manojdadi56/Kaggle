...
# Load evaluation module dynamically to handle Kaggle environment
eval_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '../../eval'))
if os.path.exists(os.path.join(eval_dir, 'cv.py')):
    sys.path.append(eval_dir)
    import cv
else:
    # Fallback/mock for Kaggle where eval dir might not be present
    class MockCV:
        @staticmethod
        def evaluate_cv(predictions, gold_data):
...
class LinearDecayLRSchedule:
    ...
    def get_lr(self, step: int, total_steps: int, epoch: int, total_epochs: int) -> float:
        mult = min(1.0, max(0.0, 1.0 - step / total_steps))
